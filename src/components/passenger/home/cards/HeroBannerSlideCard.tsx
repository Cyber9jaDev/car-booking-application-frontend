  ...
  return (
    <div className="h-screen w-screen flex-shrink-0 relative">
      ...
      <div className="absolute inset-0 flex flex-col justify-center items-center text-white p-4 bg-black/30 mr-4">
        ...
        {subtitle && <p className="text-3xl md:text-4xl lg:text-5xl font-extrabold text-center max-w-2xl">{subtitle}</p>}
        <button className="mt-12" >
          <Link className="flex text-base border-2 text-white py-5 px-8 rounded-sm font-extrabold uppercase hover:bg-[#ffc107] transition-colors duration-1000 ease-in-ou" href="/booking"> Book Now </Link>
        </button>
      </div>
    </div>
  );
